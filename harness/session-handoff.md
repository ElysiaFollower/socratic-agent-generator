...
- 当前功能项：无 active；`dreamingrag-memory-integration`、`dreamingrag-public-api-adapter` 与 `default-dreamingrag-deployment-docs` 均已标记为 `passing`
- 当前计划：无 active；已归档 `plans/archive/20260511-dreamingrag-memory-adapter-prototype.md`、`plans/archive/20260513-dreamingrag-public-api-adapter.md` 和 `plans/archive/20260513-default-dreamingrag-deployment.md`
- 归档范围：RAG adapter 代码、配置、测试、harness evidence、public API 迁移、官方部署文档和归档计划；本机 smoke 工件保持 ignored。
- 分支结论：当前分支已从 DreamingRAG 内部原型对象迁移到稳定 `dreaming_rag.public_api`，并把 DreamingRAG 默认安装与配置写入官方部署路径。
...
- 官方部署文档验证：`python3 <deployment docs reference check>` 输出 `deployment_docs_reference_dreamingrag=True`；`_local/socratic-smoke-venv/bin/pip install -e ../DreamingRAG` 成功安装 `dreaming-rag-0.5.0`；DreamingRAG public API smoke 输出 `dreamingrag_public_api_ready=True`；Socratic adapter smoke 输出 `socratic_dreamingrag_context_ready=True`。
- Harness audit：`/Users/ely/.agents/skills/harness-maintainer-zh/scripts/harness-audit.sh /Users/ely/workspace/research/agent/socratic-agent-generator` 通过，0 warning。
...
- 新增 `docs/deployment.md` 作为官方部署文档，默认部署包含 sibling DreamingRAG checkout、Python 3.10、`pip install -e ../DreamingRAG`、`.env` 配置、smoke checks、启动命令和故障排查。
- 更新 `README.md`、`docs/README.zh.md`、`AGENTS.md`、`docs/overview.md`、`harness/bootstrap-contract.md`、`init.sh` 和 `requirements.txt` 注释，使默认安装路径与官方部署文档一致。
- 更新 `harness/quality.md`，把部署文档维护纳入维护队列。
...
- Socratic 后端环境若未按官方部署文档安装 DreamingRAG，adapter 会降级为空记忆，不会阻断 Tutor；默认部署已要求 `pip install -e ../DreamingRAG`。
...
- 测试/端到端：focused unittest、前端 smoke、CLI real smoke、Tutor resumed-session memory smoke、DreamingRAG editable install smoke 和部署文档检查已通过；完整 Web 端到端未跑。
...
1. 提交并推送 `rag-memory-adapter` 分支；后续可在该基线上处理真实模式性能、异步写入、错误观测和更完整的 Web E2E。
...
- 默认安装：`pip install -e ../DreamingRAG`
