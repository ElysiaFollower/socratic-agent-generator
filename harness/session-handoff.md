...
- 用户可在 Settings 中维护实验机配置，创建会话时选择一台自己的机器。
...
- 后端调试 API 覆盖文件列表/上传、remote-put、remote-command、remote-audits，便于不用前端也能复现实验。
- 前端完成 Remote Machines 设置页、创建会话机器选择、聊天页 session file panel。
- 官方部署文档、`.env.example` 和架构文档已同步 Remote Runner conda 配置、session binding、command policy 和短等待策略。
